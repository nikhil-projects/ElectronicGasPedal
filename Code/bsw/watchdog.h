...
* If needed, a more detailed description can be given below */
       /**< \brief by putting a < next to the comment-start. The documentation referes to the left instead to the next line. */
...
*/
 enum eWDTEnum{
    WDT_2_TICKS = CYWDT_2_TICKS,             /*    4 -    6 ms */
    WDT_16_TICKS = CYWDT_16_TICKS,                /*   32 -   48 ms */
    WDT_128_TICKS = CYWDT_128_TICKS,               /*  256 -  384 ms */
    WDT_1024_TICKS = CYWDT_1024_TICKS               /* 2048 - 3072 ms */
} ;
typedef enum eWDTEnum WDT_TimeOut_t;
...
typedef enum ebitfieldEnum runnable_bitfield_t;
...
struct  sRunnable{
    ...
    volatile uint8_t run_calcControl:1;
    volatile uint8_t run_setEngine:1;
    volatile uint8_t run_setBrakeLight:1;

};
typedef struct sRunnable sRunnable_t;
